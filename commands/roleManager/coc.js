exports.run = async (client, message, args, Discord, config, errChannel) => {
  ...
  if (config.deleteCoC == `y`) message.delete(config.deleteTime * 1000);
  //commando: -cr feste add @Smoker
  var rollen = ``;
  var clans = config.clansCoC;
  ...
  let fam = message.guild.roles.get(`640769319885733899`);
  let game = message.guild.roles.get(`640769974054551558`);
  ...
  if (aktionname === `neu`) {
    ...
    if (message.member.roles.has(clanrat.id) && message.member.roles.has(rat.id)) {
      ...
      if (member.roles.has(fam.id)) {
        ...
        privat(`Rolle wurde hinzugefügt!`, `651639100801024000.png`, `Wilkommen im Clan **${clan.name}**! Da du ja bereits in der **${fam.name}** warst, muss ich ja nicht viel erklären und wünsche dir viel Spass und Erfolg! \n\n__Deine Clanführung:__ \n${fuhrungPing}`);
      ...
      privat(`Willkommen bei der Bergclan Familie!`, `651639100801024000.png`,
...
Hier ist die CoC Community vereint. Der Bereich bietet einen [Chat](https://discordapp.com/channels/375761445184471050/640774007985274900), [Freundeslink Kanal](https://discordapp.com/channels/375761445184471050/651631584218316810), u.v.m...
...
__Deine Clanführung:__ \n${fuhrungPing}`
  ...
  if (aktionname === `kick`) {
    ...
    if (message.member.roles.has(clanrat.id) && message.member.roles.has(rat.id)) {
      ...
      privat(`Hallo ${member.displayName}`, `651639100801024000.png`, `Du wurdest auf **${freunde.name}** zurückgestetzt. Wenn du wieder einem unserer Clans beitreten möchtest, nimm Kontakt zu jmd. aus der Clanführung auf: \n\n${fuhrungPing}`);
  ...
  if (aktionname === `rat`) {
    ...
    if (message.member.roles.has(clanrat.id) && message.member.roles.has(rat.id)) {
      ...
      if (!member.roles.has(altenrat.id)) {
        ...
        privat(`Willkommen im ${altenrat.name}!`, `651639100801024000.png`, `Deine Aufgabe besteht darin, deinen Clan am laufen zu halten, inaktive Spieler auszusortieren (der Clanleitung melden), aktiv an der Clangestaltung mitzuwirken und den Clanchat zu beleben! Ein bisschen wie ein Animateur, aber eher einfach ein gutes Vorbild :wink:
  ...
  if (aktionname === `clanrat`) {
    ...
    if (message.member.roles.has(clanrat.id) && message.member.roles.has(rat.id)) {
      ...
      if (!member.roles.has(clanrat.id)) {
        ...
        privat(`Willkommen im ${clanrat.name}`, `651639100801024000.png`, `Der Clanrat besteht aus Leader und einem Vize, aus jedem Clan! Ihr seid die **Chef-Ältesten**, achtet auf die anderen Ältesten und handelt übergeordnet! Hier wird über Verbundbeitritte oder Clan Neugründungen diskutiert und entschieden!
...
Im <#506914804280328192> könnt ihr ungestört miteinander sprechen falls nötig. :wink:`);
      ...
      if (member.roles.has(clanrat.id)) {
        ...
        privat(`Hallo ${member.displayName}`, `651639100801024000.png`, `Du wudest degradiert und bist nun kein **${clanrat.name}** mehr! Bei Fragen wende dich an die Clanführung: \n\n${fuhrungPing}`);
      ...
      for (const key in leitung) {
        ...
        client.users.get(userID).send(`Huhu... \n**${message.member.nickname}** hat versucht, **${member.displayName}** als **${clanrat.name}** hinzuzufügen/zu entfernen, ist aber nicht berechtigt. Diese Nachricht wurde auch an deine anderen Vize gesendet, ihr wisst also alle Bescheid. Bitte schaut euch das mal an, ggf. nhizufügen/entfernen! \nGeh in <#612953942871244800> \nDort fügst du folgende Zeile ein! \n\n:arrow_down: Kopiere :arrow_down:`)
          .then(client.users.get(userID).send(`-coc ${clannameS} clanrat @${user.username}#${user.discriminator}`))
          .catch(console.error);
      }
