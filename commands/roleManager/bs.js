exports.run = async (client, message, args, Discord, config, fehler, logging, fertig, warnung, privat) => {
  ...
  if (!member) return fehler(
    ...
    `z.b. -bs feste neu ${message.author}`
  );

  //++++++++++++++PASSENDE ROLLEN SUCHEN++++++++++++++//
  //++++++++++++++++++++++++++++++++++++++++++++++++++//-----------------------------------------------------------
  //--Alle CR-Rollen suchen
  let fam = message.guild.roles.get(config.famBS);
  let game = message.guild.roles.get(config.gameBS);
  ...
  for (const nr in clans) {
    if (clanname === clans[nr]) {
      var clan = message.guild.roles.find(r => r.name === config.clanbezBS + clanname);
      var rat = message.guild.roles.find(r => r.name === config.altBS + clanname);
      var clanratClan = message.guild.roles.find(r => r.name === config.ratBS + clanname);
  ...
  if (aktionname === `neu`) {
    ...
    if (message.member.roles.has(clanrat.id)){
      ...
      if (member.roles.has(fam.id)) {
        ...
        privat(`Rolle wurde hinzugefügt!`, config.emojiBS, `Wilkommen im Clan **${clan.name}**! Da du ja bereits in der **${fam.name}** warst, muss ich ja nicht viel erklären und wünsche dir viel Spass und Erfolg! \n\n`, `__Deine Clanführung:__`, `${fuhrungPing}`);
      ...
      privat(`Willkommen bei der Bergclan Familie!`, config.emojiBS,
...
**__⭐ Brawl Stars ⭐__**
Hier ist die BS Community vereint. Der Bereich bietet einen [Chat](${config.chanLink}/${message.guild.id}/523482001492279326), [Freundeslink Kanal](${config.chanLink}/${message.guild.id}/625327288724226048), [Turniere](${config.chanLink}/${message.guild.id}/618676602125156371), u.v.m...
...
Viel Spaß beim umsehen, bei Fragen einfach melden, bis bald :wink:`, `__Deine Clanführung:__`, `${fuhrungPing}`);
  ...
  if (aktionname === `kick`) {
    ...
    if (message.member.roles.has(clanratClan.id)) {
      ...
      privat(`Hallo ${member.displayName}`, config.emojiBS, `Du wurdest auf **${freunde.name}** zurückgestetzt. Wenn du wieder einem unserer Clans beitreten möchtest, nimm Kontakt zu uns auf:\n\n`, `__Deine Clanführung:__`, `${fuhrungPing}`);
  ...
  if (aktionname === `rat`) {
    ...
    if (message.member.roles.has(clanratClan.id)) {
      ...
      if (!member.roles.has(altenrat.id)) {
        ...
        privat(`Willkommen im ${altenrat.name}!`, config.emojiBS, `Deine Aufgabe besteht darin, deinen Clan am laufen zu halten, inaktive Spieler auszusortieren (der Clanleitung melden), aktiv an der Clangestaltung mitzuwirken und den Clanchat zu beleben! Ein bisschen wie ein Animateur, aber eher einfach ein gutes Vorbild :wink:
...
Im <#380061588201406465> könnt ihr ungestört miteinander sprechen :wink:`, `\u200b`, `\u200b`);
      ...
      if (!member.roles.has(rat.id)) {
        ...
        privat(`Hallo ${member.displayName}`, config.emojiBS, `Du wudest befördert und bist nun **${rat.name}**! Schau dir den Regierungsberich unterhalb des BS-Bereich an, er variiert bei den Clans!`, `__Deine Clanführung:__`, `${fuhrungPing}`);
        return logging(message.member.nickname + ` hat Rollen bearbeitet!`, config.emojiRole, `**${member.displayName}** wurde/n die Rolle/n ${rollen} zugewiesen!`);
      }
      if (member.roles.has(rat.id)) {
        ...
        privat(`Hallo ${member.displayName}`, config.emojiBS, `Du wudest degradiert und bist nun kein **${rat.name}** mehr! Bei Fragen wende dich an:`, `__Deine Clanführung:__`, `${fuhrungPing}`);
  ...
  if (aktionname === `clanrat`) {
    ...
    if (message.member.roles.has(baumeista.id)) {
      ...
      if (!member.roles.has(clanrat.id)) {
        ...
        privat(`Willkommen im ${clanrat.name}`, config.emojiBS, `Der Clanrat besteht aus Leader und einem Vize, aus jedem Clan! Ihr seid die **Chef-Ältesten**, achtet auf die anderen Ältesten und handelt übergeordnet! Hier wird über Verbundbeitritte oder Clan Neugründungen diskutiert und entschieden!
...
Im <#506914804280328192> könnt ihr ungestört miteinander sprechen falls nötig. :wink:`, `\u200b`, `\u200b`);
      ...
      if (!member.roles.has(clanratClan.id)) {
        ...
        privat(`Hallo ${member.displayName}`, config.emojiBS, `Du wudest befördert und bist nun **${clanratClan.name}**! Da du ja bereits im **${clanrat.name}** bist, muss ich ja nicht viel erklären, viel Spaß!`, `__Deine Clanführung:__`, `${fuhrungPing}`);
        return logging(message.member.nickname + ` hat Rollen bearbeitet!`, config.emojiRole, `**${member.displayName}** wurde/n die Rolle/n ${rollen} zugewiesen!`);
      }
      if (member.roles.has(clanratClan.id)) {
        ...
        privat(`Hallo ${member.displayName}`, config.emojiBS, `Du wudest degradiert und bist nun kein **${clanrat.name}** mehr! Bei Fragen wende dich an:`, `__Deine Clanführung:__`, `${fuhrungPing}`);
